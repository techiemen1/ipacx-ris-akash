...
# Example:
#   ./build_and_push_dockerhub.sh techiemen1 latest
# ==============================================================================
...
# 1. Build Services
echo -e "\n${YELLOW}🏗️ Step 1/3: Building Backend image...${NC}"
docker build -t "${DOCKER_USER}/ipacx-backend:${TAG}" ./backend

echo -e "\n${YELLOW}🏗️ Step 2/3: Building Frontend image...${NC}"
docker build -t "${DOCKER_USER}/ipacx-frontend:${TAG}" -f Dockerfile .

echo -e "\n${YELLOW}🏗️ Step 3/3: Building MWL Service image...${NC}"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo -e "\n${GREEN}🎉 Successfully published images to Docker Hub!${NC}"
