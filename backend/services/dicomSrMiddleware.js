...
class DICOMTagParser {
  static parseHeader(rawTags = {}) {
    const rawMod = String(rawTags["(0008,0060)"] || rawTags["Modality"] || "UNKNOWN").toUpperCase();
    const rawSex = String(rawTags["(0010,0040)"] || rawTags["PatientSex"] || "O").toUpperCase();
    const sex = rawSex.startsWith("F") ? "F" : (rawSex.startsWith("M") ? "M" : "O");
    ...
    return {
      ...
      modality: rawMod,
      study_description: String(rawTags["(0008,1030)"] || rawTags["StudyDescription"] || rawTags["study_description"] || ""),
      protocol_name: String(rawTags["(0018,1030)"] || rawTags["ProtocolName"] || ""),
      clinical_history: String(rawTags["ClinicalHistory"] || rawTags["ReasonForStudy"] || rawTags["history"] || "")
    };
  }

  static detectPregnancy(header, fhirData = null) {
    if (header.patient_sex !== "F") {
      return { is_pregnant: false, source: "Male/Other Patient", gestational_age_weeks: null };
    }

    const combined = `${header.study_description} ${header.protocol_name} ${header.clinical_history}`.toLowerCase();
    const obKeywords = ["gravid", "pregnancy", "pregnant", "gestation", "fetus", "fetal", "lmp", "edd", "obstetric", "anomaly scan"];
    ...
    for (const kw of obKeywords) {
      if (combined.includes(kw)) {
        ...
        return {
          ...
          source: `DICOM Metadata Indicator (${kw.toUpperCase()})`
        };
      }
    }
...
class SRAutoSyncService {
  ...
  static generateSrTableHtml(items = [], template = {}) {
    ...
    if (obItems.length > 0 || template.template_id === "OB_USG_ANOMALY_V2") {
      html += `
    ...
    // 3. GENERAL MODALITY PARAMETERS GRID
